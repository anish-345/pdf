...
class HistoryAdapter(private val items: List<Any>) :
    ...
    override fun getItemViewType(position: Int): Int {
        return if (items[position] is com.example.superfastbrowser.db.History) {
            VIEW_TYPE_HISTORY
        } else {
            VIEW_TYPE_AD
        }
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == VIEW_TYPE_HISTORY) {
            (holder as HistoryViewHolder).bind(items[position] as com.example.superfastbrowser.db.History)
        } else {
    ...
    class HistoryViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        private val titleTextView: TextView = itemView.findViewById(R.id.history_title)
        private val urlTextView: TextView = itemView.findViewById(R.id.history_url)

        fun bind(history: com.example.superfastbrowser.db.History) {
